...
const ClientsForm: React.FC = () => {
  const saveClientData = () => {
    ...
    // Перевірка, чи поля ім'я, телефон та пошта не порожні
    if (!clientName.value || (!clientPhone.value && !clientEmail.value)) {
      alert('Будь ласка, заповніть хоча б одне поле ім\'я та одне з полів номеру телефону чи пошти.');
      return;
    }
  ...
  return (
    <div>
      ...
      <form id="client-form">
        ...
        <label htmlFor="client-email">Електронна пошта:</label>
